    ...
    playlist(nullptr)
{
    ...
    this->setStyleSheet(QString("MainWindow { border-image: url(:/pictures/sky_background.jpg) 0 0 0 0 stretch stretch; }"));
    ...
    playlist->addMedia(QUrl("qrc:/sounds/background_music.mp3"));
...
void MainWindow::setupUI() {
    ...
    QPixmap starPixmap(":/pictures/star.jpg");
    if (!starPixmap.isNull()) {
        ratingIconLabel->setPixmap(starPixmap.scaled(18, 18, Qt::KeepAspectRatio, Qt::SmoothTransformation));
    } else {
        qDebug() << "Warning: Failed to load star icon from resource : /pictures/star.jpg. Using text fallback.";
...
void MainWindow::delayedStartGame() {
    ...
    if (musicPlayer && playlist && playlist->mediaCount() > 0) {
        if (musicPlayer->state() != QMediaPlayer::PlayingState) {
            musicPlayer->play();
            qDebug() << "Background music started.";
        }
    }
...
void MainWindow::restartGame() {
    ...
    if (musicPlayer) {
        musicPlayer->stop();
        qDebug() << "Background music stopped for restart.";
    }
...
void MainWindow::showGameOverMessage(QString message) {
    ...
    if (musicPlayer) {
        musicPlayer->stop();
        qDebug() << "Background music stopped on game end.";
    }

    bool fatalGameOver = !isWin && (message.contains("Ахмед") || message.contains("fired") || message.contains("закончилось топливо") || message.contains("насмерть"));
